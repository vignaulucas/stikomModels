...
def remove_unwanted_elements(image_path):
    picture = Image.open(image_path).convert('RGB')
    img_array = np.array(picture)
    img_array = img_array[:, :, ::-1]  # Convert RGB to BGR for OpenCV
    hsv = cv2.cvtColor(img_array, cv2.COLOR_BGR2HSV)  # Convert to HSV for color segmentation
    ...
    cleaned_img_array = cleaned_img_array[:, :, ::-1]  # Convert BGR to RGB for PIL
